...
public class AccountController {
	...
	public ResponseEntity<ResponseSuccessDto<List<Account>>> index() {
		...
		return ResponseEntity.status(HttpStatus.OK)
				.body(new ResponseSuccessDto<List<Account>>(HttpStatus.OK, CustomerConstants.MESSAGE_200, accounts));
	}
	...
	public ResponseEntity<?> show(@PathVariable String pathId, HttpServletRequest request) {
		...
		if (!pathId.matches("\\d+")) {
			return ResponseEntity.badRequest().body(new ResponseErrorDto<String>(request.getRequestURI(),
					request.getMethod(), HttpStatus.BAD_REQUEST, CustomerConstants.ERROR_ID_NUMERIC));
		}
		...
		if (account.isEmpty()) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ResponseErrorDto<String>(
					request.getRequestURI(), request.getMethod(), HttpStatus.NOT_FOUND, AccountConstants.MESSAGE_404));
		}

		return ResponseEntity.status(HttpStatus.OK)
				.body(new ResponseSuccessDto<Account>(HttpStatus.OK, CustomerConstants.MESSAGE_200, account.get()));
	}

	@PostMapping
	public ResponseEntity<?> store(@Valid @RequestBody AccountRequestDto accountRequestDto, BindingResult result,
		...
		Customer customer = this.customerService.getById(accountRequestDto.getCustomerId());
//		if (customer.isEmpty()) {
//			return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ResponseErrorDto<String>(
//					request.getRequestURI(), request.getMethod(), HttpStatus.NOT_FOUND, AccountConstants.MESSAGE_404));
//		}
		...
		return ResponseEntity.status(HttpStatus.CREATED).body(new ResponseSuccessDto<AccountResponseDto>(
				HttpStatus.CREATED, CustomerConstants.MESSAGE_201, accountResponseDto));
	}

	@PatchMapping("/{pathId}")
	public ResponseEntity<?> updateAccountType(@PathVariable String pathId, @Valid @RequestBody Map<String, String> fields,
			HttpServletRequest request ){
		...
		accountOptional.get().setAccountType(fields.get("accountType"));
		...
		return ResponseEntity.status(HttpStatus.OK).body(new ResponseSuccessDto<Account>(HttpStatus.CREATED,
				CustomerConstants.MESSAGE_200, accountUpdated.get()));
	}

	@PutMapping("/{pathId}")
	public ResponseEntity<?> update(@PathVariable String pathId, @Valid @RequestBody Account account,
	...
	@PostMapping("/create")
	public ResponseEntity<?> store(@Valid @RequestBody AccountCustomerRequestDto accountCustomerRequestDto,
			BindingResult result, HttpServletRequest request) {
